...
$sql = "SELECT DISTINCT(ticket_ref_code) as ticket_ref_code,
            ...
            ticket_priority, location_id, building_id, ticket_ref_no, ticket_id,
            cancelled_reason,
            DATE_FORMAT(cancelled_date_time, '%d-%m-%Y') as cancelled_date_time1 
        FROM tbl_tickets 
...
$total_cancelled = count($tickets_list);
?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>Cancelled Work Orders Report - Total (<?PHP echo $total_cancelled; ?>)</title>
...
<div class="report-container">
    ...
    <div class="no-print" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #e2e8f0; padding-bottom: 12px;">
        <div style="font-size: 12px; color: #64748b;">
            Document: <strong>Cancelled Work Orders Management Summary</strong>
        </div>
        ...
        <tbody>
            <tr>
                ...
                <td style="border: none; padding: 0; width: 50%; text-align: right; vertical-align: middle;">
                    <div style="font-size: 18px; font-weight: 700; color: #2e2e79; letter-spacing: 0.5px;">CANCELLED WORK ORDERS REPORT</div>
        ...
        <tbody>
            <tr>
                <td class="stat-box" style="width: 25%;">
                    <div class="stat-num" style="color: #b91c1c;"><?PHP echo $total_cancelled; ?></div>
                    <div class="stat-lbl">Total Cancelled Orders</div>
                </td>
                <td class="stat-box" style="width: 25%;">
        ...
        <table class="tbl-report">
            <thead>
                <tr>
                    ...
                    <th style="width: 120px; text-align: left;">Date &amp; Time</th>
                    <th style="width: 115px; text-align: left;">Ref. No.</th>
                    <th style="text-align: left;">Customer</th>
                    <th style="width: 110px; text-align: left;">Location</th>
                    <th style="width: 110px; text-align: left;">Building</th>
                    <th style="width: 75px; text-align: center;">Priority</th>
                    <th style="width: 100px; text-align: left;">Cancelled On</th>
                    <th style="width: 140px; text-align: left;">Reason</th>
                </tr>
            </thead>
            <tbody>
                <?PHP 
                if ($total_cancelled > 0) {
                    ...
                    foreach ($tickets_list as $row_t) {
                        ...
                        $c_date = !empty($row_t['cancelled_date_time1']) && $row_t['cancelled_date_time1'] != '00-00-0000' ? htmlspecialchars($row_t['cancelled_date_time1']) : '-';
                        $reason = !empty($row_t['cancelled_reason']) && $row_t['cancelled_reason'] != 'NA' ? htmlspecialchars($row_t['cancelled_reason']) : '-';
                        ?>
                        <tr>
                            ...
                            <td style="font-style: italic; color: #475569;"><?PHP echo $reason; ?></td>
                        </tr>
                        <?PHP 
                        $ctr++;
                    }
                    ...
                    <tr>
                        <td colspan="9" style="text-align: center; padding: 24px; color: #64748b; font-style: italic;">
                            No cancelled work orders found in the system.
                        </td>
                    </tr>
                    <?PHP 
                }
...
function fnExcelReport() {
    ...
    tab_text += "<tr><th colspan='9' style='background:#2e2e79;color:#ffffff;font-size:16px;padding:8px;'>CANCELLED WORK ORDERS REPORT - " + <?php echo json_encode(date("d-m-Y")); ?> + "</th></tr>";
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        ...
        txtArea1.contentWindow.document.execCommand("SaveAs", true, "Cancelled_Work_Orders_Report.xls");
        document.body.removeChild(txtArea1);
    } else {
        var a = document.createElement('a');
        var data_type = 'data:application/vnd.ms-excel,' + encodeURIComponent(tab_text);
        a.href = data_type;
        a.download = "Cancelled_Work_Orders_Report_" + <?php echo json_encode(date("d_m_Y")); ?> + ".xls";
