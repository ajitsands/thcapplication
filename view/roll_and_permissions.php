...
<head>
	...
	<link href="assets/css/sandsbaloon.css" rel="stylesheet" type="text/css">
	<!-- Permission CSS-->
...
$("#selected_user").css("overflow-y", "auto");
	...
	<script src="assets/js/sandsbaloon.js" type="text/javascript"></script>
...
$('#btn_popup_new_role').click(function(){
   sandsBaloon(this, '#popupWindow');
});
...
$('#myButtons').click(function(){
    $('#loadingWrapper').show();
    var txt_newusertype = $('#txt_newusertype').val();
    if(txt_newusertype=="")
    {
       setupDropdown('dropdownContent', 'error', "Please Add a User Type !", 'click'); 
       $('#loadingWrapper').hide();
       return false;
    }
    else
    {
        $.post('user_permission/permission_class/class_permission.php',
        { action:"addRolesOrGroups", add_RolesOrGroups:txt_newusertype },
        function(result){
            if(result=="New record created successfully")
            {
                setupDropdown('dropdownContent', 'success', result, 'click'); 
                location.reload();
            }
            else
            {
                setupDropdown('dropdownContent', 'error', result, 'click'); 
                $('#loadingWrapper').hide();
                $('#txt_newusertype').val('');
                return false;
            }
