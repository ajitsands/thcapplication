   $(document).ready(function() {
        ...
        var listOfAvlModules = $('#tlb_listOfAvlModules').DataTable({
            
                    "paging": false,
                    "info": false,
                    "language": { search: '', searchPlaceholder: "Search..." }
        });
        function functionlistOfAvlModules(rollID)
        {
             listOfAvlModules.destroy();
             listOfAvlModules = $('#tlb_listOfAvlModules').DataTable({
                    "paging": false,
                    "info": false,
                    "language": { search: '', searchPlaceholder: "Search..." },
                    "ajax": {
                        ...
                        'url': UrlPostclassPath,
                        'data': {
                            action: 'listOfAvlModules',
                            selectedRoleID :rollID,
                        },
                    ...
                    "initComplete": function(settings, json) {
                        if (isDataTableEmpty('#tlb_listOfAvlModules')) {
                                // DataTable is empty
                        ...
                        listOfAvlModules.rows().every(function () {
                                var data = this.data();
                                var currentRowID = data.ids;
             ...
             listOfControlsAndModules = $('#tlb_listOfControlsAndModules').DataTable({
                ...
                "columns": [
                    ...
                    { "data": "id", "width": "1%",
                        render: function ( data, type, rows, meta )
						 {
						     return '<input type="checkbox" class="form-control list-checkbox" id="list_checkbox_'+data+'"  style="width:12px;height:12px;" />';
						 }
                    },  
                    { "data": "name"},
                    { "data": "module_id","visible": false}
                ...
                "initComplete": function(settings, json) {
                        addedListOfControlsAndModules.rows().every(function () {
                            ...
                            var checkboxId = '#list_checkbox_' + currentRowID;
                            $(checkboxId).prop('checked', true);
                        });
                        $('#loadingWrapper').hide();  
                }
    ...
    //     });     
        ...
        $('#tlb_listOfRolls').on('click', 'tbody tr', function() {
            ...
            functionlistOfAvlModules(rowwId);
            addedListOfControlsAndModules = $('#tlb_addedListOfControlsAndModules').DataTable({
                    ...
                    "initComplete": function(settings, json) {
                        ...
                        // secondTableData.each(function(row) { //,"visible": false
        ...
        function checkCount(checkStatus) {
            var checkedCount = $('#tlb_listOfControlsAndModules .checkbox:checked').length;
            return checkStatus ? --checkedCount : ++checkedCount;
        }
        ...
        $('#tlb_listOfControlsAndModules').on('click', 'tbody tr', function() {
            ...
            var rowData  = listOfControlsAndModules.row($row).data();
                ...
                var rowChecked = $(this).find('.checkbox').prop('checked');
                // //alert(checkCount(rowChecked));
                // if(checkCount(rowChecked)==1)
                // {
                //     //var rowdata = tbl_listOfModules.row().data();
                //     var selectedData = tbl_listOfModules.rows('.activeTableRowColor').data();
                    
                    
                //     var rowData = JSON.parse(JSON.stringify(selectedData['0']));
                   
                //     var newData = {
                //         "id": rowData.ids,
                //         "name": rowData.module_name,
                //         "class_name": rowData.module_classview_id
                //     };
                //     addedListOfControlsAndModules.row.add(newData).draw();
        ...
        $('#btn_confim_privilages').click(function() {
            
            // var checkedCount = 0;
            var firstColumnData = [];
            var moduleID = [];
            // listOfControlsAndModules.rows().every(function () {
            //     var checkboxId = '#checkbox_' + this.data().id;
            ...
            // // if(checkedCount!==0 && module_classview_id!==undefined && module_classview_id!==0)
            // // {
            // //     firstColumnData.push(module_classview_id);   
            ...
            addedListOfControlsAndModules.rows().every(function(rowIdx, tableLoop, rowLoop) {
                var rowData = this.data(); 
                var firstColumnValue = rowData['id'];
                firstColumnData.push(firstColumnValue); 
                var module_ids = rowData['module_id'];
                moduleID.push(module_ids); 
            ...
            //console.log("module ids---"+firstColumnData);
            $.post(UrlPostclassPath,{action:'save_privilages',privilage_data:firstColumnData, rollId : rowwId,module_id:moduleID}, function(ret){
        ...
        $('#ModalClosePermission').on('click', function() {
        ...
        var tbl_listOfModules = $('#tbl_listOfModules').DataTable({
                ...
                "columns": [
                    ...
                    { "data": "ids",
                        render: function ( data, type, rows, meta )
					    {
					        return '<input type="checkbox" class="form-control checkbox" id="checkbox_'+data+'"  style="width:13px;height:13px;" />';
					    }
                    },
                    { "data": "module_name" },
                    { "data": "module_classview_id","visible": false }
                ],
                "initComplete": function(settings, json) {
                    $('#tbl_listOfModules tbody tr').on('click', function() {
                        ...
                        module_id = rowData.ids;
                        module_classview_id = rowData.module_classview_id;
                        console.log("Rpw Data : "+module_id+'---'+module_classview_id+'---'+rowwId+'------'+isChecked);
                        
                        if(isChecked)
                        {
                            if(parseInt(module_classview_id)!=0)
                            {
                                $.post(UrlPostclassPath,{action:'checkModuleOn',v_module_id:module_id,v_module_classview_id:module_classview_id,v_roleID:rowwId}, function(ret){
                                   
                                     setupDropdown('dropdownContent','success','Module Selected','click');
                                     $('#loadingWrapper').hide();  
                                });
                            }
                        ...
                        else
                        {
                            if(parseInt(module_classview_id)!=0)
                            {    
                                $.post(UrlPostclassPath,{action:'checkModuleOff',v_module_id:module_id,v_module_classview_id:module_classview_id,v_roleID:rowwId}, function(ret){
                                   
                                     setupDropdown('dropdownContent','success','Module Deselected','click');
                                     $('#loadingWrapper').hide();  
                                });
                            }
                        }
                        ...
                        $(this).addClass('activeTableRowColor');
                        load_listOfControlsAndModules('listData', module_id);
                    });
                }
        }); 
       ...
       $('#btn_selectall_submodules').click(function(){  
