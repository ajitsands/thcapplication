       ...
       let UrlPostclassPath = PermissionClassNamespace.varUrlPostclassPath;
        var dataTable = $('#dataTable').DataTable({
                "ajax": {
                    ...
                    'data': {
                        action: 'listData_developer_view'  
                    }
                },
                "columns": [
        ...
        $('#dataTable tbody').on('dblclick', 'tr', function() {
            ...
            swal({
			    title: "Are you sure?",
			    text: "Do you want to Delete this Control Name?",
			    icon: "warning",
			    buttons: true,
			    dangerMode: true,
			}).then((willDelete) => {
			   if (willDelete) {
                    ...
                    $.ajax({
                            ...
                            success: function(response) {
                              setupDropdown('dropdownContent','info',response,'click');
                              dataTable.row($('#dataTable')).remove().draw(false); 
                              dataTable.ajax.reload();
                            },
                            error: function(xhr, status, error) {
                                setupDropdown('dropdownContent','error',error,'click');
         ...
         function checkColumnValue(columnIndex, value) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (cellValue == value) {
                    ...
                    setupDropdown('dropdownContent','error',"Value exists in column " + columnIndex + " in row " + i,'click');
        ...
        $(".addNewEventstoJS").click(function() {
            ...
            if(selectModuleId=="")
            {
                setupDropdown('dropdownContent','error',"Please select a module !",'click');
                return false;
            }
            ...
            if(validationStatus!==true)
            {
                ...
                setupDropdown('dropdownContent','error',validationStatus,'click');
                return false;
            }
            ...
            if(status===true)
            {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {action: 'AddNewPermission',dbValue: moduleOrControlName, selectModuleId:selectModuleId },
                    success: function(response) {
                       
                        setupDropdown('dropdownContent','info',response,'click');
                        dataTable.ajax.reload();
                        $('#moduleOrControlName').val('');
                        $('#selectModuleId').val('').trigger("change");
                    },
                    error: function(xhr, status, error) {
                        console.error("Error appending to file:", error);
