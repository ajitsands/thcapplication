	...
	<tbody>
		...
		<tr>
		 ...
		 <?PHP 
			...
			if($customer_id === 'All')
			{
				$pr_child_table = mysqli_query($varDBConnection, "select *,DATE_FORMAT(date_of_visits, '%d-%m-%Y') as date_of_visits1,DATE_FORMAT(visit_start_time, '%d-%m-%Y %H:%i')as visit_start_time from  tbl_visits where amc_ticket='TKT' and date_of_visits between '".$start_date."' and '".$end_date."' and amc_visit_status='Cancelled' order by date_of_visits asc");
			}
			else
			{
				$pr_child_table = mysqli_query($varDBConnection, "select *,DATE_FORMAT(date_of_visits, '%d-%m-%Y') as date_of_visits1,DATE_FORMAT(visit_start_time, '%d-%m-%Y %H:%i')as visit_start_time from  tbl_visits where amc_ticket='TKT' and date_of_visits between '".$start_date."' and '".$end_date."' and amc_visit_status='Cancelled' and customer_id = '".$customer_id."' order by date_of_visits asc");
			}
