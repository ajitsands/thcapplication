    ...
    <div align="center" style="border: none; width: 1000px; margin: 0 auto;">
        <div style="border: none; display: flex; justify-content: space-between;">
            ...
            <div style="border: none; color: #daa505; text-align: right; font-weight: 700; padding-right: 20px;">
                <button id="exportExcelButton" class="btn btn-success btn-sm exportToExcelAction" onclick="exportToExcel()">Export to Excel</button>
            </div>
        </div>
    </div>
...
<table align="center" style="border: none;" width="1000">
	<tbody>
		...
		<tr style="border: none;">
		<td style="border: none;font-size: 15px;font-weight: 700;"><b> </b></td>
		<td style="text-align: right;border: none;"><b>Date & Time:</b> <?PHP date_default_timezone_set('Asia/Bahrain'); echo date("d-m-Y H:i:s");  ?></td>
		</tr>
	</tbody>
</table>
<?php  
$result_date = mysqli_query($varDBConnection,"SELECT visit_date,DATE_FORMAT(visit_date, '%d-%M-%Y') as visit_date_new FROM  tbl_ticket_teams where ticket_team_status='Active' and visit_date >= '".$_GET['start_date']."' and visit_date <='".$_GET['end_date']."' group by visit_date ");
$num_rows_date = mysqli_num_rows($result_date);
 	if($num_rows_date>0)
 {
	...
	<tbody>
     <tr style="border: none;">
		<td style="border: none;font-size: 15px;font-weight: 700;"><b>Daily Team Report : <?PHP echo date("d-m-Y",strtotime($row_date['visit_date']));  ?></b></td>
		</tr>
	</tbody>
</table>
     <?php
	...
	<!--	<tr>
			...
			<td width="255"><?PHP //echo date("d-m-Y",strtotime($row_date['visit_date']));  ?></td>
...
<p></p> 
 ...
 {?>
	...
	<tbody>
		...
		<tr>
			<td bgcolor="#2e2e79" colspan="4" style="color: #ffffff"><strong>No Data Found...</strong></td>
		</tr>
		</tbody>
		</table>
<?php } ?>
