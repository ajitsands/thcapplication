				...
				<b></b>	<a data-toggle="collapse" class="text-white" href="#collapsible-item-nested-date-<?php echo $dsl;?>"><?PHP echo $row_date['visit_date_new']; ?></a></b>
					...
					<div class="header-elements ">
							<div class="list-icons">
		                		<a data-toggle="tooltip" data-placement="bottom" title="Print Team List" href="../view/daily_reports/daily_team_list_print.php?visit_date=<?php echo $row_date['visit_date'];?>"  target="_blank" class="list-icons-item" data-action="fullscreen" ></a>
			...
			<div id="collapsible-item-nested-date-<?php echo $dsl;?>" class="collapse ">
				<div class="card-body">
				    <?PHP 	
				    	...
				    	$result = mysqli_query($varDBConnection,"SELECT GROUP_CONCAT( distinct(employee_name)) as Emp_name, GROUP_CONCAT( distinct(employee_id)) as Emp_id,visit_id,ticket_ref_no,ticket_id,amc_ticket FROM  tbl_ticket_teams where ticket_team_status='Active' and visit_date = '".$row_date['visit_date']."' group by ticket_id");
 ...
 ?>
 	...
 	<div class="card">
			...
			<div id="collapsible-item-nested-<?php echo $dsl.'-'.$i;?>" class="collapse ">
                ...
                $result_entries = mysqli_query($varDBConnection,"SELECT ticket_ref_no,ticket_id,amc_ticket,visit_id FROM  tbl_ticket_teams where ticket_team_status='Active' and visit_date = '".$row_date['visit_date']."' and employee_id in (".$row['Emp_id'].") and ticket_ref_no='".$row['ticket_ref_no']."' group by ticket_id");
                 $j=0;
                ...
                ?>
					...
					<div class="card">
						<div class="card-header header-elements-inline bg-dark">
							<h6 class="card-title">
								<a data-toggle="collapse" class="text-white" href="#collapsible-item-nested-child1_<?php echo $j.'-'.$i;?>">
							...
							//	echo 'Ref. No. : '.$row_entries['ticket_ref_no']; ?></a>
							</h6>
							<div class="header-elements">
										<div class="list-icons">
