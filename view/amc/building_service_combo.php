...
if(!empty($cus_id)) {
    $result_building = mysqli_query($varDBConnection,"select DISTINCT location_id,asset_building from  tbl_assets where customer_id='".mysqli_real_escape_string($varDBConnection, $cus_id)."'");
} else {
    $result_building = mysqli_query($varDBConnection,"select DISTINCT location_id,asset_building from  tbl_assets");
}
