...
class ticketController
{
    ...
    function __construct()
	{
        ...
        $this->visit_date_old = $_POST['visit_date_old'];
        $this->visit_slot_old = $_POST['visit_slot_old'];
        $this->total_slot_old = $_POST['total_slot_old'];
    ...
    { 
        $array =  array();
         ...
         $array[1] = "select * from  tbl_tickets where ticket_ref_code='".$this->ticket_ref_code."' and ticket_status='Assigned'";
         $array[2] = "select *,DATE_FORMAT(date_of_visits, '%d-%m-%Y') as date_of_visits,date_of_visits as date_of_visits1 from  view_ticket_visits where amc_tkt_ref_no='".$this->ticket_ref_code."' and ticket_status='Assigned'";
         $array[3] = "select * from  tbl_ticket_services where ticket_id=".$this->ticket_id." ";
         $array[4] = "select *, DATE_FORMAT(date_of_visits,'%d-%m-%Y') as date_of_visits,date_of_visits as date_of_visits1 from   tbl_visits where amc_tkt_id=".$this->ticket_id." and amc_ticket='TKT' and amc_visit_status  in ('Assigned') ";
           ...
           $array[24] ="UPDATE  tbl_ticket_services set ticket_service_status='Closed' where amc_ticket='TKT' and ticket_ref_code='".$this->ticket_ref_code."'";
            $array[25] ="UPDATE  tbl_visits set amc_visit_status='Cancelled',amc_schedule_color='#B23CFD' where amc_ticket='TKT' and amc_tkt_ref_no='".$this->ticket_ref_code."'";
             $array[26] ="UPDATE  tbl_visits set amc_visit_status='Closed',amc_schedule_color='#4CAF50' where amc_ticket='TKT' and amc_tkt_ref_no='".$this->ticket_ref_code."'";
              $array[27] ="UPDATE  tbl_ticket_teams set ticket_team_status='Deactive' where amc_ticket='TKT' and ticket_ref_no='".$this->ticket_ref_code."'";
             
            //   $array[28] ="UPDATE  tbl_visits set amc_visit_status='Cancelled', amc_schedule_color='#B23CFD' where amc_ticket='TKT' and amc_tkt_id=".$this->ticket_id;
            //   $array[29] ="UPDATE tbl_tickets set ticket_status='Cancelled',cancelled_by_id=". $this->user_id.",cancelled_by_name='".$this->username."',cancelled_reason='".$this->cancel_reason."',cancelled_date_time='".$this->createddatetime."' where ticket_id=".$this->ticket_id;
            //   $array[30] ="UPDATE  tbl_ticket_teams set ticket_team_status='Deactive' where amc_ticket='TKT' and ticket_id='".$this->ticket_id."'";
              $array[31] = "select * from  tbl_ticket_teams where visit_id=".$this->visit_id." and ticket_id=".$this->ticket_id." and  ticket_ref_no='".$this->ticket_ref_code."' and amc_ticket='TKT'  and ticket_team_status='Active'";
        ...
        switch ($FunctionEvents)
        {
             ...
             break;
               ...
               case 'update_schedules':
                  for ($i=0;$i<$this->ticket_count; $i++) {
                    ...
                    mysqli_query($this->varDBConnection,"update tbl_visits set date_of_visits='".$this->visit_date."',time_of_visit='".$this->visit_slot."',additional_slots=".$this->visit_duration.",visit_start_time='".$this->visit_start_time."',amc_schedule_color='#39C0ED',amc_visit_status='Scheduled' where amc_ticket='TKT' and  amc_visit_id=".$this->visit_id[$i]);
                    mysqli_query($this->varDBConnection,"update tbl_tickets set ticket_status='Scheduled' where ticket_id=".$this->ticket_id[$i]);
                    mysqli_query($this->varDBConnection,"delete from tbl_ticket_teams  where amc_ticket='TKT' and  visit_id=".$this->visit_id[$i]);
                    
                      for($l=$this->visit_slot_old[$i];$l<=$this->total_slot_old[$i];$l++)
                    {
                        // echo 'update tbl_technician_slots set slot_'.$l.'="0" where slot_'.$l.'="'.$this->ticket_ref_code.'" and slot_date="'.$this->visit_date_old[$i].'"';
                         mysqli_query($this->varDBConnection,'update tbl_technician_slots set slot_'.$l.'="0" where slot_'.$l.'="'.$this->ticket_ref_code.'" and slot_date="'.$this->visit_date_old[$i].'"');
                    }
             ...
             break;
              case 'cancel_team':
                  ...
                  $result =  mysqli_query($this->varDBConnection,"select amc_tkt_id,amc_visit_id from tbl_visits  where amc_ticket='TKT' and date_of_visits='".$this->visit_date."' and time_of_visit='".$this->visit_slot."' and  amc_tkt_ref_no='".$this->ticket_ref_code."' and additional_slots<=".$this->visit_duration);
                   while($row=mysqli_fetch_assoc($result)) {
                       mysqli_query($this->varDBConnection,"update tbl_visits amc_schedule_color='#39C0ED',amc_visit_status='Scheduled' where amc_ticket='TKT' and  amc_visit_id=".$row['amc_visit_id']);
                        mysqli_query($this->varDBConnection,"update tbl_tickets set ticket_status='Scheduled' where ticket_id=".$row['amc_tkt_id']);
                         mysqli_query($this->varDBConnection,"delete from tbl_ticket_teams  where amc_ticket='TKT' and  visit_id=".$row['amc_visit_id']);
                       
                   }
                    for($l=$this->visit_slot;$l<=$this->total_slots;$l++)
                    {
                       
                         mysqli_query($this->varDBConnection,'update tbl_technician_slots set slot_'.$l.'="0" where slot_'.$l.'="'.$this->ticket_ref_code.'" and slot_date="'.$this->visit_date.'"');
                    }
             ...
             case 'change_status_ticket':
              if($this->status=='Cancelled')
              {
                    ...
                    for($i=1;$i<=24;$i++)
                    {
                         mysqli_query($this->varDBConnection,'update tbl_technician_slots set slot_'.$i.'="0" where slot_'.$i.'="'.$this->ticket_ref_code.'"' );
                    }
              }
              else
              {
