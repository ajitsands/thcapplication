    ...
    { 
        ...
        $array[7] = "select * from   tbl_services where service_status='Active' and category_id=".$this->category_id." and asset_type_id=".$this->type_id;
         ...
         $array[12] = "select * from  tbl_ticket_services where ticket_id=".$this->ticket_id." and ticket_service_status='Pending'";
          ...
          $quote_date_sql = (empty($this->quote_date) || $this->quote_date == '0000-00-00' || $this->quote_date == 'undefined') ? "NULL" : "'".$this->quote_date."'";
          $date_needed_sql = (empty($this->date_needed) || $this->date_needed == '0000-00-00' || $this->date_needed == 'undefined') ? "NULL" : "'".$this->date_needed."'";
          $array[19] ="UPDATE  tbl_tickets set additional_info='".$this->additional_info."',complaints_description='".$this->complaint_description."',ticket_priority='".$this->priority_val."',quote_required='".$this->quote_val."',service_request='".$this->service_request."',job_category='".$this->job_category."',quote_date=".$quote_date_sql.",date_needed=".$date_needed_sql.",ticket_image='".$this->v_session_image."',ticket_image2='".$this->v_session_image2."',quote_ref_no='".$this->quote_ref_no."' where ticket_id=".$this->ticket_id;
           ...
           $array[25] = "INSERT INTO tbl_visits( amc_tkt_id,amc_tkt_ref_no,amc_ticket,customer_id,customer_code,customer_name,visit_mode,date_of_visits,time_of_visit,amc_visit_status) (select ticket_id,ticket_ref_code,'TKT',customer_id,customer_code,customer_name,'YSD','".$this->visit_date."','".$this->visit_time."','Scheduled' from tbl_tickets where ticket_ref_code='".$this->ticket_ref_code."' and ticket_status='Opened')";
            ...
            $array[28] = "select employee_id,employee_code,employee_name,employee_contact_no from tbl_technician_slots where slot_date='".$visit_date_val."' " . $visit_slot_clause . " and employee_id in (select employee_id from tbl_employees where employee_status='Active') and employee_code not in (select employee_code from tbl_employee_leave where employee_code IS NOT NULL AND employee_code != '' AND DATE(start_time) <= '".$visit_date_val."' and DATE(end_time) >= '".$visit_date_val."') union select employee_id,employee_code,employee_name,employee_contact_no from tbl_employees where employee_type_name in ('Technician','Team Leader','Supervisor') and employee_status='Active' and employee_code not in (select employee_code from tbl_employee_leave where employee_code IS NOT NULL AND employee_code != '' AND DATE(start_time) <= '".$visit_date_val."' and DATE(end_time) >= '".$visit_date_val."') and employee_id not in (select employee_id from tbl_technician_slots where slot_date='".$visit_date_val."') group by employee_id";
