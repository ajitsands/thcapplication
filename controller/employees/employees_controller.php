        ...
        switch ($FunctionEvents)
        {
           ...
           // echo $var[2];
            ...
            case 'fetch_leave_calendar':
                ...
                $sql = "SELECT s.leave_id, 'short' AS table_source, s.employee_code, s.employee_name, s.leave_type, s.leave_reason, s.leave_start_date AS start_date_raw, s.leave_end_date AS end_date_raw, CONCAT(s.employee_name, ' - ', s.leave_type) AS title, s.leave_start_date AS start, DATE_ADD(s.leave_end_date, INTERVAL 1 DAY) AS end, COALESCE(lt1.leave_type_color, CASE WHEN s.leave_type = 'Sick Leave' OR s.leave_reason LIKE '%Sick%' THEN '#ef5350' WHEN s.leave_type = 'Casual Leave' OR s.leave_reason LIKE '%Casual%' THEN '#42a5f5' WHEN s.leave_type = 'Annual Leave' OR s.leave_reason LIKE '%Annual%' THEN '#66bb6a' WHEN s.leave_type = 'Emergency Leave' OR s.leave_reason LIKE '%Emergency%' THEN '#ffa726' WHEN s.leave_type = 'Privilege Leave' OR s.leave_reason LIKE '%Privilege%' THEN '#ab47bc' ELSE '#26a69a' END) AS color FROM tbl_employee_short_leave s LEFT JOIN tbl_employees e ON (s.employee_code = e.employee_code OR s.employee_id = e.employee_id) LEFT JOIN tbl_leave_types lt1 ON s.leave_type = lt1.leave_type_name $where1 UNION ALL SELECT l.leave_id, 'leave' AS table_source, COALESCE(e.employee_code, l.employee_code) AS employee_code, COALESCE(e.employee_name, l.employee_name) AS employee_name, CASE WHEN l.leave_type IS NOT NULL AND l.leave_type != 'NA' AND l.leave_type != '' AND l.leave_type != 'select' THEN l.leave_type WHEN l.leave_reason LIKE '%Sick%' THEN 'Sick Leave' WHEN l.leave_reason LIKE '%Casual%' THEN 'Casual Leave' WHEN l.leave_reason LIKE '%Annual%' THEN 'Annual Leave' WHEN l.leave_reason LIKE '%Emergency%' THEN 'Emergency Leave' WHEN l.leave_reason LIKE '%Privilege%' THEN 'Privilege Leave' ELSE 'Leave' END AS leave_type, l.leave_reason, DATE_FORMAT(l.start_time, '%Y-%m-%d') AS start_date_raw, DATE_FORMAT(l.end_time, '%Y-%m-%d') AS end_date_raw, CONCAT(COALESCE(e.employee_name, l.employee_name), ' - ', CASE WHEN l.leave_type IS NOT NULL AND l.leave_type != 'NA' AND l.leave_type != '' AND l.leave_type != 'select' THEN l.leave_type WHEN l.leave_reason LIKE '%Sick%' THEN 'Sick Leave' WHEN l.leave_reason LIKE '%Casual%' THEN 'Casual Leave' WHEN l.leave_reason LIKE '%Annual%' THEN 'Annual Leave' WHEN l.leave_reason LIKE '%Emergency%' THEN 'Emergency Leave' WHEN l.leave_reason LIKE '%Privilege%' THEN 'Privilege Leave' ELSE 'Leave' END) AS title, DATE(l.start_time) AS start, DATE_ADD(DATE(l.end_time), INTERVAL 1 DAY) AS end, COALESCE(lt2.leave_type_color, CASE WHEN l.leave_type = 'Sick Leave' OR l.leave_reason LIKE '%Sick%' THEN '#ef5350' WHEN l.leave_type = 'Casual Leave' OR l.leave_reason LIKE '%Casual%' THEN '#42a5f5' WHEN l.leave_type = 'Annual Leave' OR l.leave_reason LIKE '%Annual%' THEN '#66bb6a' WHEN l.leave_type = 'Emergency Leave' OR l.leave_reason LIKE '%Emergency%' THEN '#ffa726' WHEN l.leave_type = 'Privilege Leave' OR l.leave_reason LIKE '%Privilege%' THEN '#ab47bc' ELSE '#26a69a' END) AS color FROM tbl_employee_leave l LEFT JOIN tbl_employees e ON (l.employee_code = e.employee_code OR l.employee_name = e.employee_name) LEFT JOIN tbl_leave_types lt2 ON (l.leave_type = lt2.leave_type_name) $where2";
