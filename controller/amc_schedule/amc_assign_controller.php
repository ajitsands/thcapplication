...
class amcscheduleController
{
    ...
    { 
        ...
        $leave_exclusion = " and employee_code not in (select employee_code from tbl_employee_leave where date(start_time) <= '".$this->visit_date."' and date(end_time) >= '".$this->visit_date."') ";

		$array[6] = "select employee_id,employee_code,employee_name,employee_contact_no from tbl_technician_slots where slot_date='".$this->visit_date."' and ".$this->visit_slot." and employee_id in (select employee_id from tbl_employees where employee_status='Active' and technician_type in ('Floating','NA')) " . $leave_exclusion . " union select employee_id,employee_code,employee_name,employee_contact_no from tbl_employees where employee_type_name in ('Technician','Team Leader') and employee_status='Active' and technician_type in ('Floating','NA') and employee_id not in (select employee_id from tbl_technician_slots where slot_date='".$this->visit_date."') " . $leave_exclusion . " group by employee_id";
		
		$array[7] = "select employee_id,employee_code,employee_name,employee_contact_no from tbl_technician_slots where slot_date='".$this->visit_date."' and ".$this->visit_slot." and employee_id in (select employee_id from tbl_employees where employee_status='Active' and technician_type not in ('Floating','NA')) " . $leave_exclusion . " union select employee_id,employee_code,employee_name,employee_contact_no from tbl_employees where employee_type_name in ('Technician','Team Leader') and employee_status='Active' and technician_type not in ('Floating','NA') and employee_id not in (select employee_id from tbl_technician_slots where slot_date='".$this->visit_date."') " . $leave_exclusion . " group by employee_id";
