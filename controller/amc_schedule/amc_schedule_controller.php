...
class amcscheduleController
{
    ...
    { 
		...
		$array[5] ="SELECT *,DATE_FORMAT(amc_signed_date, '%d-%m-%Y') as amc_signed_date,DATE_FORMAT(amc_start_date, '%d-%m-%Y') as amc_start_date,DATE_FORMAT(amc_end_date, '%d-%m-%Y') as amc_end_date ,CONCAT('../httpdocs/images/amc_attachements/',amc_attachment1) as amc_attachment1,CONCAT('../httpdocs/images/amc_attachements/',amc_attachment2) as amc_attachment2,CONCAT('../httpdocs/images/amc_attachements/',amc_attachment3) as amc_attachment3,amc_start_date as amc_start_dates,amc_end_date as amc_end_dates FROM `tbl_amc_master` where amc_status in ('Active','Completed') order by amc_id desc";
