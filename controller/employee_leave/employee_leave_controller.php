    ...
    function RequestAccept($FunctionEvents)
    {
        ...
        switch ($FunctionEvents)
        {
            ...
            case 'employee_on_leave_list':
                ...
                $query = "SELECT s.leave_id, 'short' AS table_source, COALESCE(e.employee_code, s.employee_code) AS employee_code, COALESCE(e.employee_name, s.employee_name) AS employee_name, COALESCE(ut1.user_type_name, e.employee_type_name, 'Staff') AS employee_type_name, s.leave_type, COALESCE(s.leave_reason, '') AS leave_reason, DATE_FORMAT(s.leave_start_date, '%d-%m-%Y') AS start_time, DATE_FORMAT(s.leave_end_date, '%d-%m-%Y') AS end_time, COALESCE(lt1.leave_type_color, '#26a69a') AS leave_type_color FROM tbl_employee_short_leave s LEFT JOIN tbl_employees e ON (s.employee_code = e.employee_code OR s.employee_id = e.employee_id) LEFT JOIN tbl_user_types ut1 ON e.employee_type_id = ut1.user_type_id LEFT JOIN tbl_leave_types lt1 ON s.leave_type = lt1.leave_type_name $where1 UNION ALL SELECT l.leave_id, 'leave' AS table_source, COALESCE(e.employee_code, l.employee_code) AS employee_code, COALESCE(e.employee_name, l.employee_name) AS employee_name, COALESCE(ut2.user_type_name, e.employee_type_name, 'Staff') AS employee_type_name, CASE WHEN l.leave_type IS NOT NULL AND l.leave_type != 'NA' AND l.leave_type != '' AND l.leave_type != 'select' THEN l.leave_type WHEN l.leave_reason LIKE '%Sick%' THEN 'Sick Leave' WHEN l.leave_reason LIKE '%Casual%' THEN 'Casual Leave' WHEN l.leave_reason LIKE '%Annual%' THEN 'Annual Leave' WHEN l.leave_reason LIKE '%Emergency%' THEN 'Emergency Leave' WHEN l.leave_reason LIKE '%Privilege%' THEN 'Privilege Leave' ELSE 'Leave' END AS leave_type, COALESCE(l.leave_reason, '') AS leave_reason, DATE_FORMAT(l.start_time, '%d-%m-%Y') AS start_time, DATE_FORMAT(l.end_time, '%d-%m-%Y') AS end_time, COALESCE(lt2.leave_type_color, '#26a69a') AS leave_type_color FROM tbl_employee_leave l LEFT JOIN tbl_employees e ON (l.employee_code = e.employee_code OR CONCAT(l.employee_code, '-', l.employee_name) = e.employee_code OR e.employee_code LIKE CONCAT('%', l.employee_code, '%') OR l.employee_name = e.employee_name) LEFT JOIN tbl_user_types ut2 ON e.employee_type_id = ut2.user_type_id LEFT JOIN tbl_leave_types lt2 ON (l.leave_type = lt2.leave_type_name) $where2 ORDER BY leave_id DESC";
