    ...
    function RequestAccept($FunctionEvents)
    {
        ...
        switch ($FunctionEvents)
        {
            ...
            case 'employee_on_leave_list':
                ...
                $query = "SELECT s.employee_code, s.employee_name, COALESCE(s.leave_type, s.leave_reason) AS leave_reason, DATE_FORMAT(s.leave_start_date, '%d-%m-%Y') AS start_time, DATE_FORMAT(s.leave_end_date, '%d-%m-%Y') AS end_time FROM tbl_employee_short_leave s LEFT JOIN tbl_employees e ON (s.employee_code = e.employee_code OR s.employee_id = e.employee_id) $where1 UNION ALL SELECT COALESCE(e.employee_code, l.employee_code) AS employee_code, COALESCE(e.employee_name, l.employee_name) AS employee_name, CASE WHEN l.leave_type IS NOT NULL AND l.leave_type != 'NA' AND l.leave_type != '' AND l.leave_type != 'select' THEN l.leave_type WHEN l.leave_reason LIKE '%Sick%' THEN 'Sick Leave' WHEN l.leave_reason LIKE '%Casual%' THEN 'Casual Leave' WHEN l.leave_reason LIKE '%Annual%' THEN 'Annual Leave' WHEN l.leave_reason LIKE '%Emergency%' THEN 'Emergency Leave' WHEN l.leave_reason LIKE '%Privilege%' THEN 'Privilege Leave' ELSE COALESCE(l.leave_reason, 'Leave') END AS leave_reason, DATE_FORMAT(l.start_time, '%d-%m-%Y') AS start_time, DATE_FORMAT(l.end_time, '%d-%m-%Y') AS end_time FROM tbl_employee_leave l LEFT JOIN tbl_employees e ON (l.employee_code = e.employee_code OR l.employee_name = e.employee_name) $where2";
