        ...
        switch ($FunctionEvents)
        {
           ...
           case 'close_ticket':
                   if($this->close_image!='NA')
                   {
                    ...
                    mysqli_query($this->varDBConnection,"update  tbl_visits set amc_visit_status='Closed',amc_schedule_color='#4CAF50',amc_service_report_no='".$this->service_report_no."',amc_service_report_uploaded_by_code='".$this->username."',amc_service_report_uploaded_date_time='".$this->current_date."',amc_close_remarks='".$this->closed_reason."', amc_service_report_image='".$this->close_image."',amc_close_code='".$this->username."', amc_close_date_time='".$this->current_date."' where amc_ticket='AMC' and amc_visit_id=".$this->amc_visit_id);
                       
                    }
             
                   else
                   { mysqli_query($this->varDBConnection,"update  tbl_visits set amc_visit_status='Closed',amc_schedule_color='#4CAF50',amc_service_report_no='".$this->service_report_no."',amc_close_remarks='".$this->closed_reason."', ,amc_close_code='".$this->username."', amc_close_date_time='".$this->current_date."' where amc_ticket='AMC' and amc_visit_id=".$this->amc_visit_id);
                   }
