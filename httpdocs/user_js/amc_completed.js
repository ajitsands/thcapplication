                 ...
                 function load_data_to_grid_amc_details_list()
                 {
                    ...
                    list_of_amc.destroy();
                     ...
                     list_of_amc = $('#list_of_amc').DataTable( {
                             ...
                             "ajax": {
                                 'type': 'POST',
                                 'url': '../controller/amc_completed/amc_completed_controller.php',
                                 'data': {
                                    action: 'amc_list'
                   ...
                   { "data": "time_of_visit",
                    ...
                    { "data": "amc_visit_id",
                         render: function ( data, type, rows, meta ) {
                            ...
                            return str_actions='<div class="list-icons"><div class="dropdown"><a href="#" class="list-icons-item" data-toggle="dropdown"><i class="icon-menu9"></i></a><div class="dropdown-menu dropdown-menu-right"><a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal_close_wo" name="amc_close_wo"><i class="icon-enter6"></i> Close WO.</a><a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal_view_services" name="amc_view_services"><i class="icon-eye"></i> View Services</a><a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal_view_assigned_team" name="amc_view_team"><i class="icon-collaboration"></i> View Team</a></div></div></div>';
...
$('#list_of_amc_schedules tbody').on( 'click', 'tr', function () {
    ...
    $('#list_of_amc_schedules tbody').on('click', 'a', function(e){
       ...
       if($(this).attr("name")=='amc_view_services')
         ...
         $('#span_ticket_ref_no_completed_view_services').html(data.amc_ref_no);
             $.ajax({
        		type: "POST",
        		url: "amc_completed/services_list_completed_modal.php",
       ...
       if($(this).attr("name")=='amc_view_team')
          ...
          if($(this).attr("name")=='amc_close_wo')
         {
           ...
           $('#span_amc_ref_no_close_modal').html(data.amc_ref_no);
           $('#txt_amc_ref_no_close').val(data.amc_ref_no);
           $('#txt_amc_visit_id_close').val(data.amc_visit_id);
 ...
 $("#close_ticket_st").click(function(){
		...
		var amc_ref_no=$('#txt_amc_ref_no_close').val();
		var amc_visit_id=$('#txt_amc_visit_id_close').val();
	 
    
         
		
		if($.trim(amc_visit_id)==0)
        {
            swal("Warning","Please select the WO to close...", "warning");
            return false;
        }
		if($.trim(service_report_no)=="")
        {
            swal("Warning","Please specify the provide the service report no...", "warning");
            return false;
        }
        else
        {
						...
						.then((willDelete) => {
						  if (willDelete) {
							var v_session_image1=$('#txt_hidden_ticket_image_close').val();
                		        $.post("../controller/amc_completed/amc_completed_controller.php",{action:'close_ticket',amc_visit_id:amc_visit_id,closed_reason:close_remarks,close_image:v_session_image1,amc_ref_nos:amc_ref_no,service_report_no:service_report_no}
