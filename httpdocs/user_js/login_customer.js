$(document).ready(function(){
   ...
   // var v_but_login = $( '#btn_sign_in' ).ladda();
     ...
     $( '#btn_login' ).click(function(){
				...
				$.post("../controller/login/login_customer_controller.php",{action:'login',v_username:v_username,v_password:v_password}, function(result,status){
					//alert(result);
	...
	//reset password starts
	 $( '#btn_reset_password' ).click(function(){
		...
		{
		   	$('body').find('.jGrowl').attr('class', '').attr('id', '').hide();
            $.jGrowl("Please fill old the fields!......",{
                position: 'top-center',
                theme: 'bg-info',
                header: 'Error',
				hideAfter: 6000
		...
		{
			$('body').find('.jGrowl').attr('class', '').attr('id', '').hide();
            $.jGrowl("Password and confirm password mismatched!......",{
                position: 'top-center',
                theme: 'bg-info',
                header: 'Error',
				hideAfter: 6000
			...
			//$('#txt_old_password').val('');
				...
				$.post("../controller/login/login_customer_controller.php",{action:'reset_password',v_password:v_new_password,v_user_id:v_user_id}, function(result,status){
