...
$result_feedback_category = mysqli_query($varDBConnection, "SELECT contract_type_name,contract_type_id,customer_name,customer_code FROM  tbl_amc_master where amc_ref_no='".$amc_ref_no."'");
while ($row_feedback_category = mysqli_fetch_assoc($result_feedback_category)) {
    $category=$row_feedback_category['contract_type_name'];
    $cate=$row_feedback_category['contract_type_name'];
    $cate_id=$row_feedback_category['contract_type_id'];
    $contract_type=$row_feedback_category['contract_type_name'];
    $customer_code = $row_feedback_category['customer_code'];
    $main_customer_name = $row_feedback_category['customer_name'];
}
$sql_cate = "SELECT * FROM feedback_questions where category_id=".$cate_id." and status='Active'";


$result_feedback_questions = mysqli_query($varDBConnection, $sql_cate);
//var_dump($result_feedback_options_data);
?>
...
<div class="container-lg">
    <div class="row" style="padding: 10px;">
        <div class="col-12 d-flex align-items-center justify-content-center">
            <img src="https://sianlab.com/thc/view/global_assets/images/logo_print_new.png" height="200" alt="...">
        </div>
        <div class="col-12 d-flex align-items-center justify-content-center">
    ...
    <?php
    $questionNumber = 1;
    while ($row_amc_details = mysqli_fetch_assoc($result_feedback_questions)) {
        $question_name = $row_amc_details['question_text'];
        $question_type = $row_amc_details['type'];
        $question_id = $row_amc_details['id'];
    ...
    function displaySingleSelection($questionNumber, $question_id, $question_name, $row_amc_details)
    {
        ...
        $DBConnMew = new DBConnection();
        $varDBConnectionNew = $DBConnMew->ConnectToMYSQL();
        $result_feedback_options_data = mysqli_query($varDBConnectionNew, "SELECT * FROM feedback_options where question_id=".$question_id);
                ...
                <div class="question">
                    ...
                    <div class="question-options card-footer">
                        <div class="row">
                            ...
                            $index = 0;
                            while ($options_data = mysqli_fetch_assoc($result_feedback_options_data)) {
                               
                                ?>
                                <?php if(is_numeric($options_data['option_text'])){?>
                                ...
                                <?php }?>
                                    <input type="radio" 
                                           ...
                                           value="<?php echo $options_data['id']; ?>"
                                           autocomplete="off">
                                           <?php if(is_numeric($options_data['option_text'])){?>
                                        ...
                                        <?php } else {?>
                                                    <label class="btn btn-primary" style="text-align:left;padding:10px;margin:10px;padding-left:20px;"
                                                           for="<?php echo 'q' . $question_id . '_' . $questionNumber . '_option' . $index; ?>"
                                                           style="font-size: 60px;"><i class="bi bi-<?php echo $index + 1; ?>-circle"></i>&nbsp;&nbsp; <small style="font-size:30px; "><?php echo $options_data['option_text']; ?></small>  </label> 
                             ...
                             mysqli_free_result($result_feedback_options_data);
                              mysqli_close($varDBConnectionNew);
                            ?>
                        </div>
                    </div>
                </div>
                             ...
                             while ($options_data = mysqli_fetch_assoc($result_feedback_options_data)) {
                                ...
                                <div class="col-12" style="padding-top:20px;">
                                    ...
                                    <label class="btn btn-primary" style="text-align:left;padding:10px;margin:10px;padding-left:20px;"
                                           ...
                                           style="font-size: 40px;width:100%;"><i class="bi bi-check"></i> <?php echo $options_data['option_text']; ?></label>
                                </div>
                               <?php
                                $index = $index+1;
                             }
                    ...
                    <div class="comment-textarea card-footer">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <textarea class="form-control" style="font-size: 40px;"
    ...
    <script>
        $(document).ready(function () {
         ...
         $('#submitBtn').click(function () {
           ...
           $.post("feedback_v1.php",{action:"insert_feedback",radioValues:v_radioValues,checkboxValues:v_checkboxValues,textValues:v_textValues,cutomerName:cutomerName,customerEmail:customerEmail,customerPhone:customerPhone,amc_ref_no:amc_ref_no,contract_type:contract_type,customer_code:customer_code,main_customer_name:main_customer_name,category:cate},function(result){
               console.log(result);
               if(result=='SuccessSuccess')
               {
                   sweetAlert("Thanks", "Thank you for your valuable feedback..!", "success");
               }
               else
               {
                   sweetAlert("Oops...", "No Feedback given", "warning");
               }
