 ...
 */
 ...
 if($_GET['ctr']==0)
 {
      $sql = "SELECT part_number FROM inventory_item_part_no where inventory_master_id=".$_GET['inventory_master_id'];
 }
 else
 {
      $sql = "SELECT part_number FROM inventory_item_part_no where inventory_item_id=".$_GET['inventory_item_id'];
 }
...
$pdf->SetTitle('iCareFon');
$pdf->SetSubject('QR Code Printing');
$pdf->SetKeywords('QR Code Printing iCareFon');
...
for($i=1;$i<=$k;$i++)
{
    ...
    $pdf->write2DBarcode($part_number[$ctr], 'QRCODE,L', 10, 3.2,2,2,  $style, 'N');
    $pdf->write2DBarcode($part_number[$ctr], 'QRCODE,L',  50, 3.2,2,2,  $style, 'N');
    //$pdf->write2DBarcode($part_number[$ctr+1], 'QRCODE,L', 50, 3.8,2,2,  $style, 'N');
    
    $pdf->Text(5.5,0.3, $part_number[$ctr]);
    $pdf->Text(46.5,0.3, $part_number[$ctr]);
