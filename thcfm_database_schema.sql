...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `acc_proc_book_ticket_new`(IN `customer_id` INT, IN `customer_code` VARCHAR(200), IN `customer_name` VARCHAR(2000), IN `location_id` INT, IN `location_code` VARCHAR(200), IN `location_name` VARCHAR(2000), IN `building_id` INT, IN `building_code` VARCHAR(200), IN `building_name` VARCHAR(2000), IN `asset_id` INT, IN `asset_code` VARCHAR(200), IN `category_id` INT, IN `category_name` VARCHAR(1000), IN `type_id` INT, IN `type_name` VARCHAR(2000), IN `additional_info` TEXT, IN `complaint_description` TEXT, IN `priority_val` VARCHAR(200), IN `quote_val` VARCHAR(200), IN `ticket_ref_val` INT, IN `ticket_ref_code` VARCHAR(200), IN `created_date_in` DATE, IN `created_date_time_in` DATETIME, IN `created_id_in` INT, IN `created_by_name` VARCHAR(200), IN `service_request` VARCHAR(200), IN `job_category` VARCHAR(200), IN `quote_date` DATE, IN `date_needed` DATE, IN `v_session_image` VARCHAR(5000), IN `v_quote_ref_no` VARCHAR(1000), IN `v_session_image2` VARCHAR(5000), IN `v_contatct_no` VARCHAR(200), IN `entry_through` VARCHAR(20), OUT `msg` VARCHAR(200), OUT `p_ids` INT, OUT `t_ids` INT)
    NO SQL
BEGIN
    DECLARE new_ticket_ref_val INT;
    DECLARE new_ticket_code VARCHAR(200);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CapitalizeWords`(IN `input_string` TEXT, IN `ids` INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_amc`(IN `amc_number` VARCHAR(30))
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_add_child_amc_new`(IN `sqlString` TEXT, OUT `ret_val` VARCHAR(10))
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_add_employee_details_v1`(IN `v_employee_type_id` INT, IN `v_employee_type_name` VARCHAR(100), IN `v_employee_password` VARCHAR(20), IN `v_employee_name` VARCHAR(100), IN `v_employee_contact_no` VARCHAR(100), IN `v_employee_email_id` VARCHAR(100), IN `v_employee_address` VARCHAR(50), IN `v_employee_image` VARCHAR(100), IN `v_expertise_id` VARCHAR(20), IN `v_expertise_name` VARCHAR(1000), IN `v_cpr_no` VARCHAR(100), IN `v_blood_group` VARCHAR(100), IN `v_passport_no` VARCHAR(100), IN `v_joining_date` DATE, IN `v_cpr_expiry_date` DATE, IN `v_visa_validity_on` DATE, IN `v_is_driving_license` VARCHAR(100), IN `v_technician_type` VARCHAR(1000), IN `v_native_no` VARCHAR(100), IN `v_native_address` TEXT, IN `v_visa_type` VARCHAR(100), OUT `ret` VARCHAR(20))
BEGIN
DECLARE exit handler for sqlexception
  BEGIN
   GET DIAGNOSTICS CONDITION 1
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_add_employee_leave`(IN `v_employee_code` VARCHAR(100), IN `v_employee_name` VARCHAR(100), IN `v_leave_type` VARCHAR(100), IN `v_leave_reason` TEXT, IN `v_start_time` DATETIME, IN `v_end_time` DATETIME)
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_add_employee_short_leave`(
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_add_service_details`(IN `v_category_type_id` INT, IN `v_category_type_name` VARCHAR(1000), IN `v_category_asset_type_id` INT, IN `v_category_asset_type_name` VARCHAR(1000), IN `v_service_desc` VARCHAR(1000), OUT `v_ret` VARCHAR(20))
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_amc_update_customer_payments`(IN `v_amc_payments_ids` INT, IN `v_date_of_payment` DATE, IN `v_invoice_ref_no` VARCHAR(200), IN `v_paid_amount` DECIMAL(18,3), IN `v_paid_vat_perct` DECIMAL(18,3), IN `v_paid_vat_amt` DECIMAL(18,3), IN `v_total_paid_amt` DECIMAL(18,3), IN `v_company_closing_entry` VARCHAR(50), IN `v_description` TEXT, IN `ret` VARCHAR(20))
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_customer_payment_collection_report`(IN `customer_id` INT, IN `amc_code` VARCHAR(20))
    NO SQL
BEGIN
SET @variable = 0;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_edit_lpo`(IN `v_item_name` VARCHAR(255), IN `v_item_qty` INT, IN `v_item_unit` VARCHAR(100), IN `v_unit_price` DECIMAL(18,3), IN `v_total` DECIMAL(18,3), IN `v_discount_percent` DECIMAL(18,3), IN `v_tax_percent` DECIMAL(18,3), IN `v_grand_total` DECIMAL(18,3), IN `v_lpo_child_id` INT)
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_edit_quotation`(OUT `ret` VARCHAR(100))
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_edit_service_details`(IN `v_service_id` INT, IN `v_service_description` TEXT, IN `v_category_id` INT, IN `v_category_name` VARCHAR(1000), IN `v_asset_type_id` INT, IN `v_asset_type_name` VARCHAR(100))
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_test`(IN `invar` VARCHAR(1000), OUT `msg` VARCHAR(200), OUT `p_ids` INT)
    NO SQL
BEGIN
SET msg='hai';
 SET p_ids=1;
